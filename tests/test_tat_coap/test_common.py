...
from ttproto.tat_coap import testcases
...
from ttproto.tat_coap.templates import Opt, NoOpt, CoAP, CoAPOptionUriPath
...
class TestCommon(unittest.TestCase):
    ...
    def __get_capture(self, pcap_file, dir_from_test_dumps="dissection/coap/coap_observe"):
        pcap_dir_path = os.path.dirname(os.path.abspath( __file__ ))
        pcap_dir_path = os.path.join(pcap_dir_path, os.path.join('../test_dumps/', dir_from_test_dumps))
        return Capture(os.path.join(pcap_dir_path, pcap_file))

    def __get_convs(self, pcap_file:str, dir_from_test_dumps='analysis/preprocess/coap'):
    ...
    def test_conv_correlation_into_single_conv(self):
        ...
        # another client doing a DELETE request on /obq
        assert len(convs) == 2
        ...
        correlated_conv = CoAPTestCase.correlate(convs, TD_COAP_OBS_07.get_stimulis())[0]
        #self.__pprint_coap_conversation(correlated_conv)
    ...
    def test_conv_correlation_with_unrealated_conv_in_pcap(self):
        ...
        assert len(convs) == 5
        all_correlated_convs = CoAPTestCase.correlate(convs, TD_COAP_OBS_07.get_stimulis())
    ...
    def test_severals_convs_for_tc_in_single_pcap(self):
        ...
        conversations_tc_09 = CoAPTestCase.correlate(conversations, TD_COAP_CORE_09.get_stimulis())
        assert len(conversations_tc_09) == 2
        one_conv_tc_09 = conversations_tc_09[0]
        another_conv_tc_09 = conversations_tc_09[1]
        assert len(one_conv_tc_09) == 4
        self.__check_conv_is_tc_09(one_conv_tc_09)
        assert len(another_conv_tc_09) == 4
        self.__check_conv_is_tc_09(another_conv_tc_09)
        ...
        conversations_tc_04 = CoAPTestCase.correlate(conversations, TD_COAP_CORE_04.get_stimulis())
        assert len(conversations_tc_04) == 2
        one_conv_tc_04 = conversations_tc_04[0]
        self.__check_conv_is_tc_04(one_conv_tc_04)
        another_conv_tc_04 = conversations_tc_04[1]
        self.__check_conv_is_tc_04(another_conv_tc_04)
        assert len(another_conv_tc_04) == 2
